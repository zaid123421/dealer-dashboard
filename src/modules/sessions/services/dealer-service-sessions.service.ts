...
export type ServiceSessionStatusFilter = ServiceSessionStatus;
export type ServiceSessionTypeFilter = ServiceSessionType;

export type DealerServiceSessionsQuery = {
  page?: number;
  size?: number;
  sortBy?: string;
  /** Sort order — API uses `direction` (asc|desc). */
  direction?: "asc" | "desc";
  status?: ServiceSessionStatusFilter;
  sessionType?: ServiceSessionTypeFilter;
  locale?: string;
};

export type DealerServiceSessionsPagedMeta = {
  page: number;
  size: number;
  totalPages: number;
...
export type DealerServiceSessionsPagedResult = {
  rows: ServiceSessionRow[];
  meta: DealerServiceSessionsPagedMeta;
};
...
function buildQueryString(query: DealerServiceSessionsQuery): string {
  const {
    page = 0,
    size = 20,
    sortBy = "startedAt",
  ...
  if (sessionType) sp.set("sessionType", sessionType);
  return sp.toString();
}

/** GET /v1/service-sessions/all — paginated service sessions. */
export async function listDealerServiceSessionsPaged(
  query: DealerServiceSessionsQuery,
): Promise<DealerServiceSessionsPagedResult> {
