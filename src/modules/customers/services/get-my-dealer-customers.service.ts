...
export async function getMyDealerCustomersService(
  params: MyDealerCustomersParams,
): Promise<DealerCustomerPage> {
  try {
    const { data } = await api.get<unknown>("/v1/dealerCustomers/my-dealer", {
      params: {
        page: params.page,
        size: params.size,
